...
static NSID_INTERNER: Lazy<RwLock<Rodeo>> = Lazy::new(|| RwLock::new(Rodeo::new()));

impl NamespacedID {
    pub fn is_valid_namespace_char(chr: char) -> bool {
        ('a'..='z').contains(&chr) || ('0'..='9').contains(&chr) || "-_".contains(chr)
    }
    ...
    pub fn new_from_parts<S1, S2>(namespace: S1, path: S2) -> Result<Self, NSIDParseError>
    where
        S1: AsRef<str>,
        S2: AsRef<str>,
    {
